...
describe('report', () => {
  ...
  it('renders markdown sections', () => {
    ...
    expect(md).toContain('# Spine Check — relatório')
    expect(md).toContain('runtime alvo spine-pixi-v8 4.2')
    expect(md).toContain('## player  (player.json · player.atlas · 1 página · variantes: @0.5x)')
    expect(md).toContain('Erros 2 · Avisos 1 · Info 1 · runtime: carregou em 84 ms')
    expect(md).toContain('### Erros\n- **skeleton.region-missing** — Attachment "arm"')
    expect(md).toContain('  Dica: Reexporte o atlas.')
